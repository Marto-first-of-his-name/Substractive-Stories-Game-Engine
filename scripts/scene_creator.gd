...
func _ready() -> void:
	available_scenes = Global.game_controller.scenes
...
func display_scene_to_edit() -> void:
	...
	for cs in scene.correct_sentences:
		_recreate_correct_sentence(cs)
		
	print("loaded scene")



func _recreate_correct_sentence(cs: CorrectSentence) -> void:
	#new Hbox
	...
	print("the sentence was ", cs.sentence)
